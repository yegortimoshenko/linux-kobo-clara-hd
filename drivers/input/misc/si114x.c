...
//static DEVICE_ATTR(als_threshold_high, S_IWUSR | S_IRUGO, als_threshold_high_show, als_threshold_high_store);
static DEVICE_ATTR_RW(als_threshold_high);
/*
 * The threshold read function returns an uncompressed value (from 8bits to 16bits)
 * The value might be different from what being written to Si114X, but the difference
 * sholud be less than 3% (before revision A10). Read AN498.pdf chapter 5.4 for details.
...
//static DEVICE_ATTR(als_threshold_low, S_IWUSR | S_IRUGO, als_threshold_low_show, als_threshold_low_store);
static DEVICE_ATTR_RW(als_threshold_low);
...
//static DEVICE_ATTR(als_vis_gain, S_IWUSR | S_IRUGO, als_vis_gain_show, als_vis_gain_store);
static DEVICE_ATTR_RW(als_vis_gain);
...
//static DEVICE_ATTR(led_drive, S_IWUSR | S_IRUGO, led_drive_show, led_drive_store);
static DEVICE_ATTR_RW(led_drive);
...
//static DEVICE_ATTR(led_input_drive, S_IWUSR | S_IRUGO, led_input_drive_show, led_input_drive_store);
static DEVICE_ATTR_RW(led_input_drive);
/* TODO(cmanton) This is a temporary alias for the above 'led_input_drive' for the upper layer
   ...
   should be removed.
   */
static DEVICE_ATTR(led_drive2, S_IWUSR | S_IRUGO, led_input_drive_show, led_input_drive_store);
//static DEVICE_ATTR_RW(led_drive2);
/*
...
#ifdef CONFIG_OF
static void si114x_parse_dt(struct si114x_data *si114x)
{
	struct device_node *si114x_np;

	si114x_np = of_node_get(si114x->client->dev.of_node);
	if (!si114x_np) {
		printk("could not find si114x sub-node\n");
		return ;
	}

	si114x->pdata->pfd_gpio_int_no = of_get_named_gpio(si114x_np,"gpios",0);
	if (!gpio_is_valid(si114x->pdata->pfd_gpio_int_no)) {
		printk("%s() invalid gpio: %d\n",__func__, si114x->pdata->pfd_gpio_int_no);
		return;
	}
	printk ("%s() gpio_int %d\n",__func__,si114x->pdata->pfd_gpio_int_no);
	if (gpio_request(si114x->pdata->pfd_gpio_int_no, "si114x_irq") < 0) {
		printk("%s() Failed to request gpio %d .\n",__func__, si114x->pdata->pfd_gpio_int_no);
	}
	else
		gpio_direction_input(si114x->pdata->pfd_gpio_int_no);
	si114x->client->irq = gpio_to_irq (si114x->pdata->pfd_gpio_int_no);
}

#else
void si114x_parse_dt(struct lm3630a_chip *pchip)
{
	return NULL;
}
#endif
...
static int si114x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		si114x->pdata = kzalloc(sizeof(struct si114x_platform_data), GFP_KERNEL);
		si114x_parse_dt (si114x);
	}
	else
...
#ifdef CONFIG_OF
static const struct of_device_id si114x_dt_match[] = {
	{ .compatible = "si,si114x-als", },
	{},
};
MODULE_DEVICE_TABLE(of, si114x_dt_match);
#endif

static const struct i2c_device_id si114x_id[] = {
	{ DEVICE_NAME, 0 },
};
static const struct dev_pm_ops si114x_pm_ops = {
...
static struct i2c_driver si114x_driver = {
	...
	.driver = {
		.owner = THIS_MODULE,
		.name = DEVICE_NAME,
		.of_match_table = of_match_ptr(si114x_dt_match),
#ifdef CONFIG_PM
		.pm	= &si114x_pm_ops,
#endif
	},
};
static int si114x_init(void)
{
	return i2c_add_driver(&si114x_driver);
}
static void si114x_exit(void)
{
